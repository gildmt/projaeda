				...
				if (ordensVenda.at(j).getTitulo() == titulo && ordensVenda.at(j).getNif() != nif) {
					...
					if (ordensVenda.at(j).getPrecoMin() <= pMax) {
						if (precoTransacao * ordensVenda.at(j).getQuantidade() <= vMaxGastar) {
							Transacao t(titulo, precoTransacao, ordensVenda.at(j).getQuantidade(), data, ordensVenda.at(j).getNif(), clientes.at(i).getNif());
							...
							Transacao t(titulo, precoTransacao, n, data, ordensVenda.at(j).getNif(), clientes.at(i).getNif());
							transacoes.push_back(t);
				...
				if (ordensCompra.at(j).getTitulo() == titulo && ordensCompra.at(j).getNif() != nif) {
					...
					if (ordensCompra.at(j).getPrecoMax() >= pMin) {
						if (precoTransacao * qtd <= ordensCompra.at(j).getValorMaxGastar()) {
							Transacao t(titulo, precoTransacao, qtd, data, clientes.at(i).getNif(), ordensCompra.at(j).getNif());
							transacoes.push_back(t);
							...
							Transacao t(titulo, precoTransacao, nQuantidade, data, clientes.at(i).getNif(), ordensCompra.at(j).getNif());
							transacoes.push_back(t);
